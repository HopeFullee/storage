...
const copyFirst = slider.firstElementChild.cloneNode(true);
const copyLast = slider.lastElementChild.cloneNode(true);  // copy first and last (item) from (slider) and,
slider.insertBefore(copyLast, slider.firstElementChild);   // append two of (cloneNodes) to (slider) on opposite direction. 
slider.appendChild(copyFirst);


const sliderCounter = document.querySelector('.slider-counter');
const sliderItem = document.querySelectorAll('.slider-item'); // DON'T CHANGE LINE 
...
for(let i=0; i<sliderItem.length; i++){
	...
	itemWidth = sliderItem[i].style.width = (100 / itemLength);
}

const sliderWidth = slider.style.width = itemLength * 100 + "%";  // automatically calculate and apply width for slider
slider.style.transform = "translateX(-"+ itemWidth +"%)" // actual START POINT = 1 / (itemLength)


let sliderPosition = 2; // start from 2 to skip (cloneNode) 

const counter = document.createTextNode((sliderPosition - 1) +' / '+ (itemLength - 2)); // subtract 2 (cloneNodes) at LINE:690
sliderCounter.appendChild(counter);  // create DEFAULT (counter) node here and will be removed from LINE:748 
...
function sliderClick(e) {
	...
	if(elem.classList.contains('next-btn')) {
		...
		if(sliderPosition === itemLength - 1){ 
			sliderPosition = 1;
	...
	if(elem.classList.contains('prev-btn')) {
		...
		if(sliderPosition === 1){
			...
			slider.style.transform = "translateX(-"+ (sliderPosition - 2) * itemWidth + "%)";
		}
